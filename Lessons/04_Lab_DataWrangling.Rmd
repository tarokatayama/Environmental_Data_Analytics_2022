...
NTL.phys.data.Tuesday.Lake.1990thru1999 <- 
  NTL.phys.data %>%
...
NTL.phys.data.Tuesday.Lake.1990thru1999.july2 <- 
  NTL.phys.data %>%
...
NTL.phys.data.Tuesday.Lake.1990thru1999.july.no.na<-
  NTL.phys.data.Tuesday.Lake.1990thru1999.july %>%
  filter(depth ==0) %>%
  filter(!is.na(temperature_C)) %>%
  summarise(mean= mean(temperature_C))
#first mean is column name, second is function mean
