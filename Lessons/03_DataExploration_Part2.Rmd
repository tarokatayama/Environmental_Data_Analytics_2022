...
#
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean), bins = 10)
...
> Answer: I learned that the discharge is generally distributed near 0. There are a lot more approved data points than nonapproved. Some graphs show the date distribution of data, while others can show us the mean and distribution of the data.


How can multiple options for data exploration inform our understanding of our data?

> Answer: it lets us look at different dimensions of data
...
> Answer: generally gage height increases with discharge, but slows down to an asymptote because a stream can only be so wide. 
