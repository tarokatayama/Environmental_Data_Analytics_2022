...
colnames(gage_data) = c("agency_cd", "site_no", "datetime", "tz_cd", "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
#Tidy the data
gage_data <- gage_data %>% 
...
theURLENO <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=7&format=rdb'

#Get the data, which starts on line 30
gage_dataENO <- read.table(theURLENO,skip = 28,header=TRUE,sep='\t') 

#Update the column headers
colnames(gage_dataENO) = c("agency_cd", "site_no", "datetime", "tz_cd", "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
#Tidy the data
gage_dataENO <- gage_dataENO %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(gage_dataENO,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
## APIs
...
```{r}
#Apply the API key
census_api_key("3dc040f400810b11f8f79d226ca95cbf13f1000d", install = TRUE, overwrite=TRUE)
