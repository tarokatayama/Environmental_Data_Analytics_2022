...
#correlation matrix used to look at multiple variables, where variables may have high correlation between each other. 
## Set up
```{r, message = FALSE}
library(tidyverse)
library(agricolae)   
...
EPAairselectvariables<-EPAair %>%
  select(meanLat:Ozone) %>%
  na.omit()
EPACorr<- cor(EPAairselectvariables)
...
OzonebyAll.regression<- lm(data=EPAairselectvariables, PM2.5~ Ozone+Year+Month+meanLat+meanLng)

step(OzonebyAll.regression)

#Exercise 7: Run another regression using the variables selected on
Ozonebestregression<- lm(data=EPAairselectvariables, PM2.5~ Ozone+Month+meanLat+meanLng)
summary(Ozonebestregression)
#Exercise 6. Compare r-squared value with the one from Exercise 5. 
