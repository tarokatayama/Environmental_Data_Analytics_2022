...
#paste0 pastes different column names together.
...
ggplot(BRA_complete, aes(x=BRA_complete$Date, y=BRA_complete$Rainfall))+
  geom_point()
...
ggplot(BRA_complete, aes(x=BRA_complete$Date, y=BRA_complete$Temperature_C))+
  geom_point()
...
firstday<- day(first(BRA_complete$Date))
...
BRA_rainfall_decomp<- stl(BRA_rainfall_ts, s.window = "periodic")
plot(BRA_rainfall_decomp)

BRA_temperature_decomp<- stl(BRA_temperature_ts, s.window = "periodic")
plot(BRA_temperature_decomp)
...
BRA_rainfall_components<-
  as.data.frame(BRA_rainfall_decomp$time.series[,1:3])

BRA_temperature_components<-
  as.data.frame(BRA_temperature_decomp$time.series[,1:3])

BRA_rain_components<- mutate(BRA_rainfall_components,
                             observed= BRA_complete$Rainfall,
                             Date=BRA_complete$Date)


BRA_rainfall_trend<- Kendall::SeasonalMannKendall(BRA_rainfall_ts)

BRA_temperature_trend<- Kendall::SeasonalMannKendall(BRA_temperature_ts)
...
BRA_rainfall_trend
summary(BRA_rainfall_trend)

BRA_temperature_trend
summary(BRA_temperature_trend)
```
